    ...
    <div class="filter-row">
      <n-button-group size="small">
        <n-button v-for="t in typeOptions" :key="t.value"
          ...
          :quaternary="contentType !== t.value"
    ...
    <n-card class="section-card">
      ...
      <div v-else class="ranking-list">
        <div v-for="(item, i) in items" :key="i"
          ...
          <n-avatar :src="item.poster_url" :size="48" round class="ranking-avatar" fallback-src="" />
...
async function loadRankings() {
  loading.value = true
  try {
    const res = await statsApiV3.contentRankings({
...
.ranking-meta { font-size: 0.75rem; color: var(--text-muted); display: flex; gap: 0.25rem; flex-wrap: wrap; }
